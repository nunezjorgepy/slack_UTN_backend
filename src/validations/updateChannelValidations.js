...
export const updateChannelValidations = (form_data) => {
    const { name, description } = form_data
    let name_error = ''
    let description_error = ''

    // Validaciones para el nombre (solo si viene en el body)
    if (name !== undefined) {
    ...
    // Validaciones para la descripcion (solo si viene en el body)
